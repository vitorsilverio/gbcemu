...
public class RotateInstruction implements Instruction {
    ...
    public RotateInstruction(RotateDirection direction, Source source, Destination destination, int cycles, boolean carryFlag, boolean zeroFlag) {
    ...
    public int execute(Cpu cpu) {
        ...
        if (RotateDirection.LEFT.equals(direction)) {
            ...
            value = (value << 1) | (carryFlag ? 1 : 0);
        } else {
            bitout = (value & 0x01) != 0 ? 1 : 0;
            value = (value >> 1) | (carryFlag ? 0x80 : 0);
        }
        if (carryFlag) {
