...
public abstract class Cart implements MemorySpace, MemoryBankProvider, Stateful<CartState> {
    ...
    public Map<String, String> debugProperties() {
        ...
        properties.put("Header ROM size", String.format("%d bytes", header.getRomSizeBytes()));
        ...
        properties.put("Header RAM size", String.format("%d bytes", header.getRamSizeBytes()));
        properties.put("RAM banks", String.valueOf(ram.bankCount()));
        properties.put("Current RAM bank", String.valueOf(ram.currentBank()));
        properties.put("Header checksum", String.format("%02X", header.getHeaderChecksum()));
        properties.put("Computed header checksum", String.format("%02X", header.getComputedHeaderChecksum()));
        properties.put("Header checksum valid", String.valueOf(header.isHeaderChecksumValid()));
        properties.put("Global checksum", String.format("%04X", header.getGlobalChecksum()));
        properties.put("Computed global checksum", String.format("%04X", header.getComputedGlobalChecksum()));
        properties.put("Global checksum valid", String.valueOf(header.isGlobalChecksumValid()));
